...
newimg = False
#path, alt
newi = ["/2020/10/09/mail-in-ballot.jpg", "Mail in ballot"]
t = ["Ripon Fire Board Race Down to One Vote As Other Contests Stabilize", "ripon-fire-more-stable-races"]
l = 1
...
if newimg:
    ...
    s = '\n<div id="%scontainer" class="storycontainer bigstory"><a href="/%s/%s/%s/%s.html" class="storylink"><div class="storyimgcontainer %s"><img src="%s" alt="%s" class="storyimg"></div><div class="storytextcontainer %s"><p class="mainstory headline storyheadline">%s</p><p class="articledate">%s %d, %s</p></div></a></div>'%(id, d[0], d[1], d[3], t[1], ids[l][0], newi[0], newi[1], ids[l][1], t[0], d[2], int(d[3]), d[0])
copy(s)
