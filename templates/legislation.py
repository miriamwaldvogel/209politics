...
for j, i in enumerate(r):
    filter['date'].append(i[4])
    if i[3] not in filter['area']:
        filter['area'][i[3]] = [j]
    else:
        filter['area'][i[3]].append(j)
    s = ''
...
f.close()
areafilters = ''
for i in filter['area']:
    areafilters += """<div class="filterbox">
<input type="checkbox" id="%s" class="checkbox" onchange="filter('%s', 'area');" checked="true">
...
g = g[1].split('<div id="legislation">', 1)
f.write(g[0]+'<div id="legislation">'+p)
